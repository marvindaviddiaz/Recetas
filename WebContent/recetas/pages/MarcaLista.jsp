...
<body class="fondo">
	<f:view>
		<h:form styleClass="fondoForm">
			<h:inputHidden value="#{marcaListBean.load}" />
			<rich:panel styleClass="panelPrincipalLista">
				<h:outputText value="Listado de Marcas" styleClass="tituloPagina" />
				<br>
				...
				<h:panelGrid columns="2" width="90%">
					<rich:panelMenu styleClass="menuPanel" width="200px;" mode="ajax" iconExpandedGroup="disc" iconCollapsedGroup="disc" iconExpandedTopGroup="chevronUp" iconGroupTopPosition="right"
						...
						<rich:panelMenuGroup label="Acciones" expanded="true">
							<rich:panelMenuItem label="Agregar" action="#{marcaListBean.addAction}" oncomplete="Richfaces.showModalPanel('modalPanel');" reRender="formDetalle">
							</rich:panelMenuItem>
							<rich:panelMenuItem label="Editar" action="#{marcaListBean.editAction}" oncomplete="if ('#{facesContext.maximumSeverity.ordinal}' < 2) Richfaces.showModalPanel('modalPanel');"
								reRender="formDetalle">
							</rich:panelMenuItem>
							<rich:panelMenuItem label="Eliminar" action="#{marcaListBean.deleteAction}" reRender="panelListado">
							</rich:panelMenuItem>
					...
					<h:panelGrid id="panelListado" columns="1" cellspacing="1" cellpadding="0">
						<rich:dataTable id="listado" rowClasses="listadoDatosDetalle1 ,listadoDatosDetalle2" headerClass="listadoDatosHeader" value="#{marcaListBean.lista}" var="bean" border="0" cellpadding="3"
							...
							<rich:column sortBy="#{bean.nombre}">
								...
								<a4j:commandLink value="#{bean.nombre}" action="#{marcaListBean.loadBean}" oncomplete="Richfaces.showModalPanel('modalPanel');" reRender="formularioDatos">
		...
		<a4j:include id="formDetalle" viewId="MarcaDetalle.jsp" ajaxRendered="false" />
